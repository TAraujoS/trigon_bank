...
export const signUp = async ({ password, ...userData }: SignUpParams) => {
  ...
  try {
    ...
    console.log(userExempleBankAccount)
    const bankExempleAccount = await createBankAccount({
      accountId: ID.unique(),
      bankId: userExempleBankAccount.$id,
      mask: "1234",
      currentBalance: 1,
      accountType: "Poupança",
    })
    console.log(bankExempleAccount)
    const userExempleTransfer = await createTransaction({
      accountId: bankExempleAccount.accountId,
      category: "Transferência",
      name: "Trigon Exemplo",
      amount: "50",
      senderId: newUser.$id,
      senderBankId: userExempleBankAccount.bankId,
      receiverId: newUser.$id,
      receiverBankId: bankExempleAccount.bankId.bankId,
      email: email,
    })

    console.log(userExempleTransfer)
    cookies().set("appwrite-session", session.secret, {
      path: "/",
      httpOnly: true,
      sameSite: "strict",
